...
class ResNet(nn.Module):
    ...
    def forward(self,x, or_bboxes, **kwargs):
        """
        :param x: B * T * D * H * W
        ...
        for i in range(x.shape[0]):
            ...
            if self.add_info[0] != '0':
                tmp_x = [0] * 4
                tmp_x[kwargs['loc'][i]] = 1
                info_x_list.extend(tmp_x)
            if self.add_info[1] != '0':
                info_x_list.append(kwargs['age'][i])
            if self.add_info[2] != '0':
                tmp_x = [0] * 3
                tmp_x[kwargs['gender'][i]] = 1
                info_x_list.extend(tmp_x)
