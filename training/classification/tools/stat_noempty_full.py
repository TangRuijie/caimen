...
def mk_data_json(split_ratio = 0.2):
    ...
    with open('buffer/noempty_data_train.json','w') as f:
        json.dump(train_dict, f)
    with open('buffer/noempty_data_valid.json','w') as f:
    ...
    with open('buffer/noempty_data_test.json','w') as f:
...
def bbb_index(imgs):
    ...
    for i in range(imgs.shape[0]):
        if np.sum((imgs[i] > 0).astype('int')) > 20:
        # if np.any(imgs[i]):
            index_list.append(i)
    return index_list
