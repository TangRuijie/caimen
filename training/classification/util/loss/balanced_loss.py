...
def focal_loss(labels, logits, alpha, gamma, weight = None, reduce = True):
...
class BalancedLoss(nn.Module):
    ...
    def forward(self, logits, labels, input_weights = None, reduce = True):
        '''

        Args:
            logits:
            labels:
            input_weights: a weights vector with batch size length
        ...
        if loss_type == "focal":
            cb_loss = focal_loss(labels_one_hot, logits, weights, gamma=self.gamma, weight=input_weights,reduce=reduce)
        elif loss_type == "l1":
            cb_loss = F.l1_loss(logits,  labels_one_hot, reduce=False)
            cb_loss = (cb_loss * weights).mean()
